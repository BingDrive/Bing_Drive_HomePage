...
class TransportationApp:
    ...
    def create_login_page(self):
        ...
        ttk.Label(login_frame, text="Login to BingDrive", font=('Helvetica', 16, 'bold')).grid(row=0, column=0, columnspan=2, pady=10)

        ttk.Label(login_frame, text="Name:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.name_entry = ttk.Entry(login_frame)
        self.name_entry.grid(row=1, column=1, pady=5)

        ttk.Label(login_frame, text="Level:").grid(row=2, column=0, sticky=tk.W, pady=5)
        self.level_entry = ttk.Entry(login_frame)
        self.level_entry.grid(row=2, column=1, pady=5)

        ttk.Label(login_frame, text="Matric Number:").grid(row=3, column=0, sticky=tk.W, pady=5)
        self.matric_number_entry = ttk.Entry(login_frame)
        self.matric_number_entry.grid(row=3, column=1, pady=5)

        ttk.Label(login_frame, text="Department:").grid(row=4, column=0, sticky=tk.W, pady=5)
        self.department_entry = ttk.Entry(login_frame)
        self.department_entry.grid(row=4, column=1, pady=5)

        ttk.Button(login_frame, text="Login", command=self.login).grid(row=5, column=0, columnspan=2, pady=10)
    ...
    def login(self):
        ...
        self.root.destroy()
        root = tk.Tk()
        app = TransportationAppMain(root, self.current_user)
        root.mainloop()

class TransportationAppMain:
    ...
    def create_main_app(self):
        ttk.Label(self.root, text="BingDrive", font=('Helvetica', 16, 'bold')).grid(row=0, column=0, columnspan=2, pady=10, sticky="n")

        ttk.Label(self.root, text=f"Welcome, {self.current_user.name}!", font=('Helvetica', 12)).grid(row=1, column=0, columnspan=2, pady=5, sticky="n")
        ...
        # Adding routes
        self.add_route("ICT", "Clinic", 3, "Bus", 100)
        self.add_route("Clinic", "Portfolio", 2, "Walking", 0)
        self.add_route("Portfolio", "Chapel", 1.5, "Bicycle", 50)
        self.add_route("Chapel", "Green Plaza", 1, "Bus", 100)
        self.add_route("Green Plaza", "Clinic Road", 1, "Walking", 0)
        self.add_route("Clinic Road", "Class", 1.5, "Bicycle", 50)

        # Dropdown menu for selecting start point
        ttk.Label(self.root, text="Select Start Point:").grid(row=2, column=0, sticky=tk.W, pady=5, padx=10)
        ...
        start_point_menu.grid(row=2, column=1, pady=5, padx=10)
        start_point_menu.set("ICT")

        # Input form for entering destination
        ttk.Label(self.root, text="Enter Destination (End Point):").grid(row=3, column=0, sticky=tk.W, pady=5, padx=10)
        destination_entry = ttk.Entry(self.root, textvariable=self.destination_entry)
        destination_entry.grid(row=3, column=1, pady=5, padx=10)

        ttk.Button(self.root, text="Calculate Cost and Time", command=self.calculate_cost_and_time).grid(row=4, column=0, columnspan=2, pady=10, sticky="n")

        # Profile section
        ttk.Label(self.root, text="User Profile", font=('Helvetica', 12, 'bold')).grid(row=0, column=2, columnspan=2, pady=10, sticky="ne")
        ttk.Label(self.root, text=str(self.current_user)).grid(row=1, column=2, columnspan=2, pady=5, sticky="ne")

        # Logout button
        ttk.Button(self.root, text="Logout", command=self.logout).grid(row=2, column=2, columnspan=2, pady=10, sticky="ne")
    ...
    def calculate_cost_and_time(self):
        ...
        if route:
            ...
            cost_label.grid(row=5, column=0, columnspan=2, pady=5, sticky="n")

            distance_label = ttk.Label(self.root, text=f"Distance: {route.distance} km")
            distance_label.grid(row=6, column=0, columnspan=2, pady=5, sticky="n")

            estimated_time_label = ttk.Label(self.root, text=f"Estimated Time: {self.calculate_estimated_time(route.distance)} minutes")
            estimated_time_label.grid(row=7, column=0, columnspan=2, pady=5, sticky="n")
        else:
            ttk.Label(self.root, text="Route not found. Please check your input.").grid(row=5, column=0, columnspan=2, pady=5, sticky="n")
